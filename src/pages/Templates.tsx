...
const Templates = () => {
  ...
  // Convert workouts to template format for display
  const templates = workouts
    .filter(w => w.category === 'template')
    .map(workout => ({
      id: workout._id,
      name: workout.title,
      ...
      conversionRate: `${((workout.reps / (workout.load + 1)) * 100).toFixed(1)}%`
    }));

  // Add some default templates if no templates exist
